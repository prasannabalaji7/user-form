...
const ProfileCard :React.FC<ProfileProps> = (props) =>{
	...
	const [imgurl,setImageUrl] = useState("https://i.stack.imgur.com/YQu5k.png");
	const [popup, setPopup] = useState({modalShow:false,title:"",message:""});
    ...
    axios.post('http://localhost:3001/transferFile', fd)
  ...
  const  handleSelect=(event:any) =>{
    ...
    const fileInput = document.querySelector('input#transfer-file') as HTMLElement;
    if (fileInput) {
     fileInput.click();
    }
  }
	...
	return (
       ...
       <Card className="card text-center w-75">
		  <Card.Body>
		    <Image id="profilepic" src={imgurl} alt="laugh" roundedCircle/>
		  	<ListGroup className="list-group-flush borderless">
			    <ListGroupItem className ="borderless h5">{props.userProfileName}</ListGroupItem>
			    <ListGroupItem className ="borderless">{props.userProfileRole}</ListGroupItem>
			    <ListGroupItem className ="borderless">{props.userProfileCountry}</ListGroupItem>
			</ListGroup>
			{(!props.isEditEnabled) && 
			<div>
        ...
        onHide={() => setPopup({modalShow:false,title:"",message:""})}
        title = {popup.title}
